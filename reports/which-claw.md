...
> Derived from **kaiser-data**'s 1,399 starred repos (snapshot `2026-07-27T09:02:42.013Z`), cross-referenced with the repo-similarity graph.
>
> Generated 2026-07-27 by `scripts/reports/which_claw.py` (regenerate any time — no API cost).
...
**On raw metrics, [`zeroclaw-labs/zeroclaw`](https://github.com/zeroclaw-labs/zeroclaw) wins** (composite 0.845): health 89, bus factor 3, very active. And it's **robust** — it stays #1 under 4 of 6 weighting profiles (see the sensitivity analysis), so that's not an artifact of how I weighted the score. If you want the cleanest, most resilient standalone claw and don't care about the surrounding tooling, take it.

**As a pragmatic default, [`openclaw/openclaw`](https://github.com/openclaw/openclaw) (composite 0.774, #3).** The score above *deliberately excludes the ecosystem network effect* — and that's OpenClaw's real edge: every accessory you've already starred (`clawhub`, `ClawRouter`, `clawmetry`, `opik-openclaw`, `openclaw-supermemory`, `NemoClaw`, `moltworker`) targets OpenClaw, not zeroclaw. That's a genuine switching cost in its favour.
...
- **Maximum stability/quality →** [`zeroclaw-labs/zeroclaw`](https://github.com/zeroclaw-labs/zeroclaw) (health 89).
...
|---|---|---|---|---|---|---|---|---|---|
| 🥇 | [zeroclaw-labs/zeroclaw](https://github.com/zeroclaw-labs/zeroclaw) | General assistant | **0.845** | 32,406 (▲75) | 89 | 14,819 | 0d ago | 3 | Rust |
| 🥈 | [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent) † | General assistant | **0.790** | 221,121 (▲3,677) | 80 | 38,156 | 0d ago | 2 | Python |
| 🥉 | [openclaw/openclaw](https://github.com/openclaw/openclaw) | General assistant | **0.774** | 384,279 (▲746) | 79 | 117,661 | 0d ago | 1 | TypeScript |
| 4 | [HKUDS/nanobot](https://github.com/HKUDS/nanobot) † | General assistant | **0.741** | 46,283 (▲367) | 83 | 19,715 | 0d ago | 2 | Python |
| 5 | [sipeed/picoclaw](https://github.com/sipeed/picoclaw) | General assistant | **0.726** | 29,733 (▲45) | 84 | 12,901 | 4d ago | 2 | Go |
| 6 | [code-yeongyu/oh-my-openagent](https://github.com/code-yeongyu/oh-my-openagent) † | Coding agent | **0.715** | 66,645 (▲428) | 78 | 21,152 | 0d ago | 1 | TypeScript |
| 7 | [NVIDIA/NemoClaw](https://github.com/NVIDIA/NemoClaw) | Secure runtime | **0.711** | 21,952 (▲104) | 74 | 12,317 | 0d ago | 3 | TypeScript |
| 8 | [nearai/ironclaw](https://github.com/nearai/ironclaw) | Secure runtime | **0.701** | 12,565 (▲33) | 80 | 5,413 | 0d ago | 2 | Rust |
| 9 | [nanocoai/nanoclaw](https://github.com/nanocoai/nanoclaw) | Secure runtime | **0.682** | 30,380 (▲86) | 74 | 12,893 | 1d ago | 2 | TypeScript |
| 10 | [RightNow-AI/openfang](https://github.com/RightNow-AI/openfang) | General assistant | **0.647** | 18,064 (▲29) | 76 | 5,689 | 25d ago | 1 | Rust |
| 11 | [elizaOS/eliza](https://github.com/elizaOS/eliza) † | General assistant | **0.625** | 18,815 (▲43) | 74 | 1,320 | 0d ago | 1 | TypeScript |
| 12 | [ultraworkers/claw-code](https://github.com/ultraworkers/claw-code) | Coding agent | **0.607** | 194,930 (▲99) | 62 | 49,557 | 1mo ago | 1 | Rust |
| 13 | [nullclaw/nullclaw](https://github.com/nullclaw/nullclaw) | General assistant | **0.606** | 7,914 (▲120) | 78 | 2,367 | 8d ago | 1 | Zig |

**Where's Hermes?** [`NousResearch/hermes-agent`](https://github.com/NousResearch/hermes-agent) lands **#2** (composite 0.790) — the **strongest functional claw** and it leads OpenClaw (#3). Health 80, bus factor 2 (vs OpenClaw's 1 — more resilient), 221,121★, very active.
It sits just behind [`zeroclaw-labs/zeroclaw`](https://github.com/zeroclaw-labs/zeroclaw), which edges it on health (89 vs 80) and resilience (bus 3 vs 2). 
...
Other functional claws (†): `nanobot` #4, `oh-my-openagent` #6, `eliza` #11.
...
|---|---|---|---|---|---|---|
| zeroclaw-labs/zeroclaw | 0.89 | 0.81 | 1.00 | 0.65 | 0.82 | **0.845** |
| NousResearch/hermes-agent | 0.80 | 0.96 | 0.67 | 0.55 | 0.90 | **0.790** |
| openclaw/openclaw | 0.79 | 1.00 | 0.33 | 0.73 | 1.00 | **0.774** |
| HKUDS/nanobot | 0.83 | 0.84 | 0.67 | 0.43 | 0.85 | **0.741** |
| sipeed/picoclaw | 0.84 | 0.80 | 0.67 | 0.41 | 0.81 | **0.726** |
...
|---|---|---|---|---|---|---|---|---|
| zeroclaw | **1** | **1** | **1** | 3 | **1** | 6 | 2.2 | #1–#6 |
| hermes-agent † | 2 | 2 | 2 | 2 | 3 | 2 | 2.2 | #2–#3 |
| openclaw | 3 | 3 | 5 | **1** | 7 | **1** | 3.3 | #1–#7 |
| nanobot † | 4 | 4 | 3 | 5 | 4 | 5 | 4.2 | #3–#5 |
| picoclaw | 5 | 6 | 4 | 6 | 5 | 7 | 5.5 | #4–#7 |
| oh-my-openagent † | 6 | 5 | 8 | 4 | 9 | 4 | 6.0 | #4–#9 |
| NemoClaw | 7 | 8 | 7 | 11 | 2 | 9 | 7.3 | #2–#11 |
| ironclaw | 8 | 7 | 6 | 9 | 6 | 11 | 7.8 | #6–#11 |
| nanoclaw | 9 | 9 | 9 | 8 | 8 | 8 | 8.5 | #8–#9 |
| openfang | 10 | 10 | 10 | 10 | 10 | 10 | 10.0 | #10–#10 |
| claw-code | 12 | 13 | 13 | 7 | 13 | 3 | 10.2 | #3–#13 |
| eliza † | 11 | 11 | 11 | 12 | 11 | 13 | 11.5 | #11–#13 |
| nullclaw | 13 | 12 | 12 | 13 | 12 | 12 | 12.3 | #12–#13 |

**Read-out.**
- **`zeroclaw` is the robust #1** — first under 4 of 6 profiles, mean rank 2.2, never below #6. The top spot is *not* an artifact of the chosen weights.
...
**Pareto-optimal (9):** `zeroclaw`, `hermes-agent`, `openclaw`, `nanobot`, `picoclaw`, `nanoclaw`, `openfang`, `claw-code`, `nullclaw`.
...
In the repo-similarity graph (1,138 nodes / 4,533 edges), the claws **don't form one cluster** — they scatter across **8 of 25 communities**. There is no single 'claw' neighbourhood; these are genuinely different projects that happen to share a role.

- **Centrality (PageRank).** Most hub-like claws: `oh-my-openagent` (0.0021), `NemoClaw` (0.0012), `openfang` (0.0011). Note PageRank tracks *similarity* connectivity, not quality — a claw is central when many neighbours resemble it.
...
- **The honest network-effect caveat.** The similarity graph measures shared topics/authors, **not** 'plugs-into' dependency — so it does *not* by itself prove OpenClaw lock-in. The one direct graph signal that does is **`openclaw` ⇄ `clawhub` (its official skill directory) at w=0.72** — the strongest accessory tie of any claw. The broader lock-in argument below rests on real-world integration, which the graph under-counts, not over-counts.
...
`zeroclaw-labs/zeroclaw` edges out `openclaw/openclaw` on the composite mostly on **health (89 vs 79)** and **bus factor (3 vs 1)** — both real, both in zeroclaw's favour. But the composite scores each claw *in isolation*. It can't see that:

- Your starred ecosystem is built **around OpenClaw** — `clawhub` (skills, the strongest single graph edge at w=0.72), `ClawRouter` (routing, on-chain payments), `clawmetry` / `opik-openclaw` (observability), `openclaw-supermemory` (memory), `NemoClaw` / `moltworker` (hosting). None of that plugs into zeroclaw out of the box. (The graph under-counts this — it sees topic/author similarity, not 'plugs-into' integration — so treat the real lock-in as *stronger* than the edges suggest.)
...
| **Best health score** | [`zeroclaw-labs/zeroclaw`](https://github.com/zeroclaw-labs/zeroclaw) | health 89 — cleanest maintenance signals |
| **Fastest-growing right now** | [`openclaw/openclaw`](https://github.com/openclaw/openclaw) | ~117,661 est. stars/30d |
...
| **Most-adopted / most battle-tested** | [`openclaw/openclaw`](https://github.com/openclaw/openclaw) | 384,279★ |
...
<sub>Claws ranked: 13 · Snapshot: 2026-07-27T09:02:42.013Z · regenerate via scripts/reports/which_claw.py</sub>
