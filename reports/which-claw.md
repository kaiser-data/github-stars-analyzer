...
> Derived from **kaiser-data**'s 2,243 starred repos (snapshot `2026-09-25T10:37:19.717Z`), cross-referenced with the repo-similarity graph.
>
> Generated 2026-09-25 by `scripts/reports/which_claw.py` (regenerate any time — no API cost).
...
|---|---|---|---|---|---|---|
| openclaw/openclaw | 0.84 | 1.00 | 0.67 | 0.74 | 1.00 | **0.855** |
| NousResearch/hermes-agent | 0.80 | 0.96 | 0.67 | 0.61 | 0.92 | **0.804** |
| zeroclaw-labs/zeroclaw | 0.83 | 0.81 | 0.67 | 0.65 | 0.81 | **0.762** |
| HKUDS/nanobot | 0.84 | 0.84 | 0.67 | 0.45 | 0.84 | **0.746** |
| code-yeongyu/oh-my-openagent | 0.78 | 0.87 | 0.33 | 0.76 | 0.85 | **0.720** |
...
| hermes-agent † | 2 | 2 | 2 | 2 | 2 | 2 | 2.0 | #2–#2 |
| zeroclaw | 3 | 3 | 3 | 5 | 3 | 6 | 3.8 | #3–#6 |
| nanobot † | 4 | 4 | 4 | 4 | 5 | 4 | 4.2 | #4–#5 |
| oh-my-openagent † | 5 | 5 | 8 | 3 | 8 | 3 | 5.3 | #3–#8 |
| picoclaw | 7 | 6 | 7 | 6 | 7 | 7 | 6.7 | #6–#7 |
| NemoClaw | 6 | 8 | 6 | 10 | 4 | 9 | 7.2 | #4–#10 |
| ironclaw | 8 | 7 | 5 | 9 | 6 | 10 | 7.5 | #5–#10 |
...
- **Hermes is the stability champion of the top tier** — mean 2.0, range #2–#2; it never leaves the podium under any weighting. The most *weighting-proof* pick.
...
**Pareto-optimal (8):** `openclaw`, `oh-my-openagent`, `NemoClaw`, `picoclaw`, `nanoclaw`, `nullclaw`, `claw-code`, `openfang`.
...
In the repo-similarity graph (1,138 nodes / 7,393 edges), the claws **don't form one cluster** — they scatter across **10 of 25 communities**. There is no single 'claw' neighbourhood; these are genuinely different projects that happen to share a role.

- **Centrality (PageRank).** Most hub-like claws: `oh-my-openagent` (0.0011), `hermes-agent` (0.0007), `openclaw` (0.0006). Note PageRank tracks *similarity* connectivity, not quality — a claw is central when many neighbours resemble it.
- **Closest claw pair:** `claw-code` ⇄ `oh-my-openagent` (w=0.50) — near-substitutes. The `zeroclaw` ⇄ `openclaw` edge confirms they compete for the same slot.
- **The honest network-effect caveat.** The similarity graph measures shared topics/authors, **not** 'plugs-into' dependency — so it does *not* by itself prove OpenClaw lock-in. The one direct graph signal that does is **`openclaw` ⇄ `clawhub` (its official skill directory) at w=1.15** — the strongest accessory tie of any claw. The broader lock-in argument below rests on real-world integration, which the graph under-counts, not over-counts.
...
| **Fastest-growing right now** | [`openclaw/openclaw`](https://github.com/openclaw/openclaw) | ~96,012 est. stars/30d |
...
| **Most-adopted / most battle-tested** | [`openclaw/openclaw`](https://github.com/openclaw/openclaw) | 390,467★ |
...
- **RightNow-AI/openfang** — health 47; 85d since last push; bus factor 0 (single-maintainer risk).
...
<sub>Claws ranked: 13 · Snapshot: 2026-09-25T10:37:19.717Z · regenerate via scripts/reports/which_claw.py</sub>
