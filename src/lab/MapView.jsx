...
function ControlPanel({ settings, setSettings, onRecenter }) {
  ...
  return (
    <div className="absolute top-3 right-3 z-10 bg-gray-900/90 backdrop-blur border border-gray-700 rounded-lg p-3 text-xs text-gray-200 shadow-xl w-[260px]">
      <button onClick={onRecenter} className="w-full mb-2 px-3 py-1.5 bg-blue-600 hover:bg-blue-500 rounded text-white font-medium">Recenter view</button>
...
export default function MapView({ onSelectNode, focusedRepoName }) {
  ...
  const { status, nodes = [], links = [], stats, error } = useGraph();
  ...
  useEffect(() => {
    ...
    const ts = [setTimeout(recenter, 400), setTimeout(recenter, 1800)];
  ...
  useEffect(() => {
    if (!fgRef.current || !focusedRepoName) {
      setHighlightLinks(new Set());
      setHighlightNodes(new Set());
      return;
    }
    const node = nodes.find((n) => n.full_name === focusedRepoName);
    if (!node) return;
    ...
    for (const l of links) {
      ...
      if (sId === node.id) { neighbors.add(tId); lset.add(l); }
      else if (tId === node.id) { neighbors.add(sId); lset.add(l); }
    }
  ...
  return (
    <div className="relative">
      ...
      <div ref={containerRef} style={{ width: '100%', height: '78vh', background: 'radial-gradient(circle at 30% 20%, #1e293b 0%, #020617 70%)', borderRadius: 12, overflow: 'hidden' }}>
        {nodes.length === 0 && <div className="flex items-center justify-center h-full text-gray-500">Building graph…</div>}
        {nodes.length > 0 && (
          <ForceGraph3D
            ref={fgRef}
            width={size.w}
            ...
            nodeColor={(n) => {
              ...
              return isHighlight ? (n.color ?? '#3b82f6') : 'rgba(75,85,99,0.3)';
            }}
            nodeOpacity={0.95}
